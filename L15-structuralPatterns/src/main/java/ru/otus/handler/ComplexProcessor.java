...
public class ComplexProcessor implements Handler {
    ...
    public Message handle(Message msg) {
        ...
        for (Processor pros : processors) {
            try {
                ...
                newMsg = makeOperationsForHistory(newMsg);
            } catch (Exception ex) {
    ...
    private Message makeOperationsForHistory(Message msg) {
        if (!listeners.get("history").isEmpty()) {
            HistoryListener historyListener = (HistoryListener) listeners.get("history").get(0);
            msg = msg.toBuilder().setUniqueId(historyListener).build();
            writeToHistory(msg, historyListener);
        }
        return msg;
    }

    public static void writeToHistory(Message msg, HistoryListener historyListener) {
        historyListener.addToHistory(msg);
    }
}